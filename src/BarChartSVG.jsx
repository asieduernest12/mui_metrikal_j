...
function BarChart(props) {
	...
	React.useEffect(() => {
		if (svgRef.current === undefined) {
			...
			// Lines
			svgRef.current.append('g').attr('id', 'lines');
			// Title
			svgRef.current.append('g').attr('id', 'title');
			// Bars
			svgRef.current.append('g').attr('id', 'bars');
			// Labels
			svgRef.current.append('g').attr('id', 'labels');
			// Values
			svgRef.current.append('g').attr('id', 'values');
		}
		...
		const getLabelX = (index) =>
			config.labels.x + ((1 - config.bars.ratio + index + config.bars.ratio / 2) * config.bars.width) / (data.length + 1 - config.bars.ratio);
		...
		svgRef.current
			...
			.join(
				(enter) => {
					enter.append('text').attr('x', getTitleXValue()).attr('y', getTitleYValue()).text(title);
				},
	...
	return <div ref={containerRef} width='100%' height='100%' />;
}
